...
4. \`get_decision\` — poll until the status is no longer AWAITING_APPROVAL. Pass
   ...
   spinning. APPROVED → continue. DENIED → stop, do not check out. Any other
   status (e.g. EXPIRED, FAILED) → stop, do not check out, and report the status
   to the user — only APPROVED continues the flow. If the status is still
   AWAITING_APPROVAL after ~10 minutes of polling, stop: tell the user the
   approval request timed out and that they can ask again to retry — do not
